...
internal sealed class EmployeeService : IEmployeeService
{
    ...
    {
        var company = _repositoryManager.Company.GetCompany(companyId, trackChanges: compTrackChanges);
        if (company is null)
            throw new CompanyNotFoundException(companyId);

        var employeeEntity = _repositoryManager.Employee.GetEmployee(companyId, id, trackChanges: empTrackChanges);
        if (employeeEntity is null)
            throw new EmployeeNotFoundException(id);
